...
def _engagement_db_has_message(engagement_db, message, transaction=None):
    """
    ...
    matching_messages = engagement_db.get_messages(filter=filter, transaction=transaction)
...
def _ensure_engagement_db_has_message(transaction, engagement_db, message, message_origin):
    """
    ...
    :type engagement_db: engagement_database.EngagementDatabase
    :param message: Message to make sure exists in the engagement.
    :type message: engagement_database.data_models.Message
    :param message_origin:
    :type message_origin: dict
    """
    if _engagement_db_has_message(engagement_db, message, transaction=transaction):
    ...
    engagement_db.set_message(
        message,
        HistoryEntryOrigin(origin_name="Rapid Pro -> Database Sync", details=message_origin),
        transaction=transaction
    )


def sync_rapid_pro_to_engagement_db(google_cloud_credentials_file_path, rapid_pro_configuration,
    ...
    :param rapid_pro_configuration: Configuration for the sync operation.
    :type rapid_pro_configuration: src.rapid_pro_to_engagement_db.configuration.RapidProToEngagementDBConfiguration
    ...
    rapid_pro = init_rapid_pro_client(google_cloud_credentials_file_path, rapid_pro_configuration.domain,
                                      rapid_pro_configuration.token_file_url)
    ...
    for flow_config in rapid_pro_configuration.flow_result_configurations:
        ...
        for i, run in enumerate(runs):
            ...
            _ensure_engagement_db_has_message(engagement_db.transaction(), engagement_db, msg, history_origin)
